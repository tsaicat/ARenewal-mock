...
export async function processReply({ messageId, replyId, from, subject, plainText, receivedAt, rawSource, requestUrl }) {
  ...
  // Classify only what the person actually typed — strip any quoted
  // copy of the original offer email their client appended below the
  // reply, so our own "To accept, reply ACCEPT. To decline, reply
  // DECLINE." instructional text doesn't get read back as if the
  // customer wrote it (see lib/emailQuote.js for why this matters).
