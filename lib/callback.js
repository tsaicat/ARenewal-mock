...
import { internalCallbackToken } from "./internalCallback.js";
...
export async function postCallback({ event, targetUrl, requestUrl }) {
  ...
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Mock-Internal-Callback": internalCallbackToken(event.eventId) },
