...
import { kvGet, kvSet, kvSetIfAbsent } from "./store";
...
export async function postCallback({ event, targetUrl, requestUrl }) {
  const dedupeKey = `callback-sent:${event.eventId}`;
  ...
  if (!isFirstAttempt) {
    const prior = await kvGet(`callback-result:${event.eventId}`);
...
async function kvSet_result(eventId, result) {
  await kvSet(`callback-result:${eventId}`, result);
}
